...
import { Settings, ChevronDown, ChevronUp, FileText, MapPin, Hash, Zap } from "lucide-react"
...
interface AdvancedSearchPanelProps {
  ...
  onMatchSelect: (match: AdvancedSearchMatch) => void
  isVisible: boolean
}

export default function AdvancedSearchPanel({ documentLayout, onMatchSelect, isVisible }: AdvancedSearchPanelProps) {
  const [query, setQuery] = useState("")
  const [matches, setMatches] = useState<AdvancedSearchMatch[]>([])
  ...
  const handleSearch = async () => {
    ...
    try {
      const results = AdvancedDocumentParser.searchDocumentAdvanced(documentLayout, query, searchOptions)
      setMatches(results)
  ...
  const highlightMatch = (context: string, matchText: string, beforeContext: string, afterContext: string) => {
    const beforeHighlight = beforeContext.slice(-50)
    const afterHighlight = afterContext.slice(0, 50)
  ...
  }, [currentMatchIndex, matches.length, isVisible])
